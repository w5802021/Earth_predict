...
def read_data():
    train_X = pd.read_csv('E:\kaggle\kaggle-Lanl_Earthquake_Prediction\master_last/train_x.csv')
    train_X1 = pd.read_csv('E:\kaggle\kaggle-Lanl_Earthquake_Prediction\input\masters-final-project/train_x.csv')
    train_X1['classic_sta_lta1_mean_0'].loc[~np.isfinite(train_X1['classic_sta_lta1_mean_0'])] = \
        train_X1['classic_sta_lta1_mean_0'].loc[np.isfinite(train_X1['classic_sta_lta1_mean_0'])].mean()
    train_x = pd.concat([train_X,train_X1],axis = 1)

    test_X = pd.read_csv('E:\kaggle\kaggle-Lanl_Earthquake_Prediction\master_last/test_x.csv')
    test_X1 = pd.read_csv('E:\kaggle\kaggle-Lanl_Earthquake_Prediction\input\masters-final-project/test_x.csv')
    test_x = pd.concat([test_X, test_X1], axis=1)

    train_y = pd.read_csv('E:\kaggle\kaggle-Lanl_Earthquake_Prediction\input\masters-final-project/train_y.csv')
    # fi = np.isinf(train_X).any()
    c1 = train_X1.columns
    c1 = c1.drop(['seg_id','seg_start',	'seg_end'])
    c2 = train_X.columns

    return train_x, train_y, test_x,c1,c2
...
def xgb_trimmed_model(scaled_train_X, scaled_test_X, train_y,c1,c2):
    maes = []
    rmses = []
    tr_maes = []
    tr_rmses = []
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(scaled_train_X, train_y.values)):
        ...
        fold_importance_df = fold_importance_df.sort_values('importance')
    ...
    print(
        'Train shape: {}, Test shape: {}, Y shape: {}'.format(scaled_train_X.shape, scaled_test_X.shape, train_y.shape))

if __name__ == '__main__':
    train_X, train_y, test_X, c1, c2  = read_data()


    scaled_train_X, scaled_test_X = scale_fields(train_X,test_X)
    # scaled_train_X = scaled_train_X.dropna(inplace=0)
    # scaled_test_X = scaled_test_X.dropna(inplace=0)

    xgb_trimmed_model(scaled_train_X,scaled_test_X,train_y,c1,c2)