...
def join_mp_build():
    ...
    for i in tqdm(range(1, NUM_THREADS)):
        ...
        train_y = pd.concat([train_y,temp])

    train_X.drop(['seg_id', 'seg_start', 'seg_end'], axis=1 ,inplace=True)
    train_X = train_X.reset_index(drop=True)

    train_X_mfcc = pd.read_csv(PATH + 'input/masters-final-project/train_xmfcc60.csv')
    train_X_mfcc.drop(['seg_id', 'seg_start', 'seg_end'], axis=1 ,inplace=True)
    train_X_mfcc = train_X_mfcc.reset_index(drop=True)
    test_X_mfcc = pd.read_csv(PATH + 'input/masters-final-project/test_xmfcc60.csv')
    test_X_mfcc.drop(['seg_id'], axis=1 ,inplace=True)

    train_X = pd.concat([train_X, train_X_mfcc], axis=1)
    test_X = pd.concat([test_X, test_X_mfcc], axis=1)
...
def xgb_trimmed_model(scaled_train_X, scaled_test_X, train_y):
    ...
    sele = sele[(sele['mean'] < 0.65) & (sele['mean'] > 0)]
    sele = list(sele.index)

    features = [x for x in list(df['col']) if x in sele]

    scaled_train_X = scaled_train_X[features]
    scaled_test_X = scaled_test_X[features]
    ...
    # params = myhyperopt.quick_hyperopt(scaled_train_X, train_y, 'xgb', 2500)
...
def lgb_trimmed_model(scaled_train_X, scaled_test_X, train_y):
    ...
    #########################由pearson系数过滤特征##########################
    df = df.iloc[: 520]  # 改动  取根据pearson系数最大的前500个特征
...
if __name__ == '__main__':
    ...
    lgb_trimmed_model(scaled_train_X, scaled_test_X, train_y)