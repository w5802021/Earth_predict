...
train_big_seg = [29, 325, 689, 916, 1242, 1449, 1630, 2044,
           2247, 2494, 2787, 3070, 3297, 3517, 3895, 4138]

test_big_seg = ['seg_26a2a0','seg_724df9','seg_7848f8','seg_7a9f2b','seg_7fa6ec','seg_aa98cc','seg_b35174','seg_c80857','seg_e3d751']
...
def load_feature_data():
    ...
    train_features1 = pd.read_csv(PATH + 'data_sample/train_features.csv')
    ...
    # train_X = pd.concat([train_features,train_mfcc_features], axis=1)  #train_mfcc_features
    ...
    # for i in range(4194):
    #     if  i in train_big_seg:
    #         scaled_train_X.loc[i,'catlo'] = 1
    #     else:
    #         scaled_train_X.loc[i, 'catlo'] = 0

    # test_X = pd.DataFrame()
    test_features = pd.read_csv(PATH + 'input/lanl-features/test_features.csv')
    test_features1 = pd.read_csv(PATH + 'data_sample/test_features.csv')
    ...
    # test_X = pd.concat([test_features,test_mfcc_features], axis=1).drop(['seg_id'], axis=1) #test_mfcc_features.drop(['seg_id'], axis=1)   #
    test_X = test_features1

    scaled_test_X = pd.DataFrame(scaler.transform(test_X), columns=test_X.columns)
    submission = pd.read_csv(PATH + 'input/sample_submission.csv', index_col='seg_id')
    # ll = list(submission.index)

    # for i,id in enumerate(ll):
    #     if  id in test_big_seg:
    #         scaled_test_X.loc[i,'catlo'] = 1
    #     else:
    #         scaled_test_X.loc[i, 'catlo'] = 0
...
def run_model_lgbm(params,scaled_train_X, scaled_test_X, train_y,feature_col):
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(scaled_train_X, train_y.values)):
        ...
        clf = lgb.LGBMRegressor(**params, n_estimators = 10000, n_jobs=-1)

        clf.fit(X_tr, y_tr,eval_set=[(X_tr, y_tr), (X_val, y_val)], eval_metric='mae',verbose=1000,early_stopping_rounds=100)
...
def main(nrows=None):
    ...
    # train_X = scaled_train_X.loc[4194:,:]
    # train_Y = train_y.loc[4194:,:]
    # test_X = scaled_train_X.loc[0:4194,:]
    # test_Y = train_y.loc[0:4194, :]
    ...
    # sele = pd.read_csv('E:/kaggle/kaggle-Lanl_Earthquake_Prediction/feat_val/feat_auc_85.csv').T
    # sele.columns = ['mean', 'std']
    # sele = sele.dropna()
    # sele = sele.sort_values('mean', ascending=False)
    ...
    submit(submission, predictions,'69feat-aug5000+')
    # # res.append(mae)
    # plt_feature_importance(feature_importance)
    plt.figure(figsize=(16, 8))
    plt.plot(train_y[:4194], color='g', label='y_train')
